...
public class PCollabController implements Initializable {
  ...
  public void initialize(URL url, ResourceBundle rb) {
        ...
        new java.util.TimerTask() {
          ...
          public void run() {
            ...
            idm.setCellValueFactory(
              new PropertyValueFactory<utilisateur, String>("ID_Utilisateur")
            );
    ...
    timer.schedule(
      ...
      50,
      500
    ); //wait 50 ms before doing the action and do it evry 500ms (0.5second)
  }
  ...
  private void returnb(ActionEvent event) {
    try {
      collab_page_home =
        FXMLLoader.load(getClass().getResource("CollabListe.fxml"));
    } catch (IOException ex) {
      Logger
        .getLogger(CollabwController.class.getName())
    ...
    collab_page_home.setOnMousePressed(
    ...
    collab_page_home.setOnMouseDragged(
      new EventHandler<MouseEvent>() {
  ...
  public static void addLabel(String messageFromClient, VBox vbox) {
    ...
    TextFlow textFlow = new TextFlow(text);
    textFlow.setStyle(
      "-fx-background-color: rgb(233,223,235);" + "-fx-background-radius: 20px;"
    );
    textFlow.setPadding(new Insets(5, 10, 5, 10));
  ...
  private void gettoken(ActionEvent event) {
    ...
    WebEngine webEngine = webs.getEngine();
    String url =
      "https://trello.com/1/authorize?expiration=never&scope=read,write,account&response_type=token&name=Server%20Token&key=e0efb3bcbe6f5dbf936f8c2e883a9580";
  ...
  private void Confirmer(ActionEvent event) {
    if (detect == 1) {
      ...
      cs.supprimer_membre(
        s.getID_Collab(),
        tab.getSelectionModel().getSelectedItems().get(0).getID_Utilisateur()
      );
    ...
    if (!msg.getText().isEmpty()) {
      ...
      for (int i = 0; i < js.length(); i++) {
        ...
        for (int l = 0; l < js1.length(); l++) {
          ...
          for (int k = 0; k < 2; k++) {
            if (k < 1) mmg = mmg + j3.getString(k) + "/"; else mmg =
              mmg + j3.getString(k) + " *";
          }
        }
  ...
  private void trad(ActionEvent event) {
    if (!msg.getText().isEmpty()) {
      ...
      HttpResponse<String> response = Unirest
        ...
        .header(
          "x-rapidapi-key",
          "5220b89461msh77e120852a978d6p1540bdjsn73fa90c52972"
        )
